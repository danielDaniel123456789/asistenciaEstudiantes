function calificaCotidianoDelAyo() {
    ...
    // Abrir SweetAlert para calificar trabajo cotidiano del año
    Swal.fire({
        title: 'Calificar Trabajo Cotidiano del Año',
        html: ` 
            <select id="cotidianoGrupo" class="form-select">
                <option value="" disabled selected>Selecciona un grupo</option>
                ${grupoOptions}
            </select>
            <br>
            <select id="cotidianoMateria" class="form-select">
                <option value="" disabled selected>Selecciona una materia</option>
                ${materiaOptions}
            </select>
            <br>
            <!-- Deshabilitamos el selector de tipo de participación ya que siempre será "Participación activa" -->
            <select id="cotidianoType" class="form-select" disabled>
                <option value="3" selected>Participación activa</option>
            </select>
        `,
        focusConfirm: false,
        ...
        preConfirm: () => {
            const groupId = document.getElementById('cotidianoGrupo').value;
            const materiaId = document.getElementById('cotidianoMateria').value;
            const type = '3'; // Establecer siempre como "Participación activa"
            ...
            // Asignar el valor de participación a cada estudiante en cada fecha del año
            ...
            // Asignar el valor de participación a cada día del año para cada estudiante seleccionado
            selectedStudents.forEach(student => {
                student.trabajoCotidiano = student.trabajoCotidiano || [];

                datesInYear.forEach(month => {
                    month.days.forEach(date => {
                        student.trabajoCotidiano.push({
                            id: Date.now() + Math.floor(Math.random() * 1000), // ID único
                            type: type, // "Participación activa" por defecto
                            materiaId: materiaId,
                            grupoId: groupId,
                            date: date
                        });
        ...
        if (result.isConfirmed) {
            Swal.fire('Guardado', 'Trabajo cotidiano del año registrado correctamente para todos los estudiantes', 'success');
        }
    });
}
